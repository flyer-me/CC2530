...
/******************************************************************************
      函数说明：显示图片 设计为图片放大为原4倍像素
      入口数据：x,y    起点坐标
      返回值：  无
******************************************************************************/
void LCD_ShowPicture(u16 x1,u16 y1,u16 x2,u16 y2)
{
		int h,w,min_Height,min_Weidth;
        unsigned char i;
		LCD_Address_Set(x1,y1,x2,y2);
        //min_x指存储图像的分辨率，本程序设计为120*90，下面将对图像进行4倍放大得到240*180图像
        min_Height = (y2 - y1 + 1)/2 - 1;
        min_Weidth = (x2 - x1 + 1)/2;
       
         for(h=0;h<min_Height;h++)
	 	{
            for(i=0;i<2;i++)//像素点纵向复制:每一行像素都在下一行重复一次
            {
              for(w=0;w<min_Weidth;w++)
              {
                //像素点横向复制：每一个像素都重复一次
                LCD_WR_DATA8(image[(min_Weidth*(h + i) + w)*2 - 1]);LCD_WR_DATA8(image[(min_Weidth*(h + i) + w)*2 - 2]);//一个像素点
                LCD_WR_DATA8(image[(min_Weidth*(h + i) + w)*2 - 1]);LCD_WR_DATA8(image[(min_Weidth*(h + i) + w)*2 - 2]);//一个像素点
              }
